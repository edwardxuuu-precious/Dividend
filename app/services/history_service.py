...
class HistoryService:
    ...
    按交易日逐日计算 TTM 股息率：
        ...
        股息率_t   = TTM 分红_t / 收盘价_t * 100

    设计：
    - 静态部分（series/percentiles/annual/forecast/eod）—— 6h 文件缓存
    - 动态 current —— 永远基于最新 quote 实时计算，不缓存
    这样盘中点开详情、首页表格、估值徽章三者都同步。
    """
    ...
    def __init__(
        ...
        # 缓存名 v3：新增 lapsed_summary 字段，旧 v2 缓存自然失效
        self.static_cache = history_cache or FileCache(
            "history_static_v3_cache", _STATIC_TTL_SECONDS
        )
    ...
    def get_percentile_only(self, stock: Stock) -> dict | None:
        """主表横向对比用：仅当静态缓存已就绪时返回 P 分位 + 估值标签。
    ...
    def _compute_live_current(self, stock: Stock, static: dict) -> dict | None:
        """
        从 PriceService 拿 5s 缓存的最新 quote，结合昨收 TTM 重新算今日股息率与分位。
